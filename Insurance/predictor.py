...
from Insurance.entity.config_entity import TRANSFORMATION_OBJECT_FILE_NAME,MODEL_FILE_NAME,TARGET_ENCODER_OBJECT_FILE_NAME

# Create a floder (saved a new model)
...
# 3
    ...
    def get_latest_transformer_path(self):
        try:
            ...
            return os.path.join(latest_dir, self.transformer_dir_name,TRANSFORMATION_OBJECT_FILE_NAME)
        except Exception as e:
            raise e
...
# 4
    def get_latest_target_encoder_path(self):
        try:
            latest_dir = self.get_latest_dir_path()
            if latest_dir is None:
                raise Exception(f"Target encoder data is not avaliable")
...
# 7
    def get_latest_save_transfomer_path(self):
        try:
            latest_dir = self.get_latest_save_dir_path()
            return os.path.join(latest_dir, self.transformer_dir_name, TRANSFORMATION_OBJECT_FILE_NAME) # transform.pkl
        except Exception as e:
            raise e
